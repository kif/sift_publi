...
probably due to the strong change in intensity on the resin borders, forcing an
alignment on this line. Figure \ref{fig:alignment} highlights this by taking a
vertical section of the sample at $x=500$ and $y\in[490, 580]$
(picture coordinates of figure \ref{fig:sample}) as horizontal axis and the
frame number on the vertical axis (this is similar to the energy but
here the step was not kept constant within the energy scan).
As intensities are changing with energy in XANES, the color of the graphs
indicates the derivative of the transmission to highlight the sample boundaries.
When the alignment is perfect, maxima (red lines) and minima (blue lines) of
the derivative are prefect vertical lines; and this is not the case here for
phase correlation. Figure \ref{fig:offset} presents the measured offsets with
both methods. Offsets in horizontal direction ($\delta x$) agree within an error
...
\caption{Comparison of the alignment quality obtained from SIFT keypoints
(on the right) with phase correlation (on the left); all frames aligned on frame
11.
The color scale indicates the intensity of the derivative of the transmission for
a vertical line at $x=500, y\in[490, 580]$ in figure \ref{fig:sample}.}
\end{center}
\end{figure}
