...
algorithm, which is robust to rotation, illumination change, translation and
scaling by design, presents an additional advantage: alignment can be limited to
...
extensively been used during the development of FFXAS for stack alignment (the
second being an adaptation of the fist), however they are limited
to translation movement and turned out to be very sensitive to artifacts, such as:
...
the flat-field nomalization, they can still remain and disturb the alignment.}, etc.
...
Nevertheless this patent only applies in the United States of America
(it does not appear to be extended to other countries) and its use is allowed
for academic research purposes (personal communication from the author).
...
4.7  as a plugin for image stack alignement; providing a graphical
interface and making it easier to use.
