...
refinement) did not raise any particular difficulty. 
For these steps, we highly benefitted from the device parallelism : every pixel 
is handled by a single thread. 
Besides that, convolution is implemented in the direct space (without Fourier
transform) and can be up to 100 times faster than the convolution done in the
C++ reference implementation of IPOL \cite{ASIFT}.  
...
complexity and on the device the code is executed on. This is why we have
written different versions, each adapted to different platforms and
...
patented by the Univerity of Columbia \cite{SIFT_pat}, nevertheless this patent
...
4 Mpixel image and returned a 261 keypoints vector as a numpy array named
\emph{kp1}.
...
affine transformation: combination of translation, rotation, homothety and
reflection. An \emph{aligner} object is defined in [10] with a reference
image and the kind on device. This aligner contains keypoints of the
...
K20m. The acceleration measured on large images (more than 10 Mpixels) is
between 30 times and 100 times depending on the image complexity, on the GPU and from 4 times to 10 times when running on CPU.
